...
public class UpdateNavigation : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.name.Equals("NavTrigger(Clone)") && line.positionCount > 0)
        {
            ...
            Vector3 pos = arcoreDeviceCam.transform.position + arcoreDeviceCam.transform.forward * 3 + arcoreDeviceCam.transform.up * -0.3f ; //Vector3 pos = arcoreDeviceCam.transform.position + arcoreDeviceCam.transform.forward * 2 + arcoreDeviceCam.transform.up * -0.5f;
            // rotate arrow a bit
